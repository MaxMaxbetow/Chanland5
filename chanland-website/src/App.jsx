...
function Header() {
  ...
  return (
    ...
    <div className={`fixed z-50 w-full flex justify-center top-0 transition-all duration-300 ${isScrolled ? 'px-4' : 'px-0'}`}>
      <header 
        className={`${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'} transition-all duration-500 w-full`}
      >
        {/* Основной контейнер хедера с анимацией трансформации */}
        <div 
          ...
          style={{
            transform: `scaleX(${isScrolled ? 0.96 : 1})`,
        ...
        >
          <div className="mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 sm:px-5 py-2 sm:py-2.5 bg-white/50 hover:bg-white/70 backdrop-blur-sm rounded-full font-semibold text-slate-700 border border-white/60 shadow-lg transition-all duration-300 flex items-center gap-2 hover:scale-105 active:scale-95"
              >
              ...
              <button
                ...
                className="lg:hidden p-2 rounded-full bg-white/30 hover:bg-white/50 backdrop-blur-sm border border-white/40 transition-all duration-300 hover:scale-105 active:scale-95"
              >
          ...
          {menuOpen && (
            <div className="lg:hidden absolute top-full left-4 right-4 mt-2 bg-white/60 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-xl p-6 space-y-3 animate-in slide-in-from-top-2 duration-300">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  onClick={() => setMenuOpen(false)}
                  className="block px-4 py-3 text-slate-700 hover:text-slate-900 font-medium rounded-xl hover:bg-white/50 transition-all duration-200 hover:translate-x-1"
                >
                  {link.label}
                </a>
              ))}
              ...
              <div className="pt-2 border-t border-white/30">
                <button
                  ...
                  className="flex items-center justify-between w-full px-4 py-3 text-slate-700 font-medium rounded-xl hover:bg-white/50 transition-all duration-200"
                >
                  <span>Скачать сборку</span>
                  <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${downloadOpen ? 'rotate-180' : ''}`} />
                </button>
                ...
                {downloadOpen && (
                  <div className="overflow-hidden animate-in slide-in-from-top-2 duration-300">
                    <div className="pl-4 space-y-2 mt-2">
                      <a 
                        href="https://www.dropbox.com/scl/fo/k88tm3wivn3qmzo6bkgft/ANrJMW5tN_oaE47nokFa1mw?rlkey=m3qz2lzoylu07myg5n22541ck&st=4dn4lnz3&dl=0" 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="block px-4 py-2 text-slate-500 hover:text-slate-800 text-sm rounded-lg hover:bg-white/40 transition-all duration-200 hover:translate-x-1"
                      >
                        Сборка сезон 4
                      </a>
                      <a 
